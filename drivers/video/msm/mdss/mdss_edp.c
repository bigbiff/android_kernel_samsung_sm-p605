...
void mdss_edp_set_backlight(struct mdss_panel_data *pdata, u32 bl_level)
{
	...
	if (bl_level == 0){
		pr_err("%s : bl_level(%d) is 0..\n", __func__, bl_level);
		return;
	} else if (bl_level < MIN_BL_LEVEL) { /*even 1 or 2 bl level to make brightness work as min level  in edp on state  */
		bl_level = MIN_BL_LEVEL;
		pr_err("%s : bl_level(%d) is too low.., to make bl as MIN(3)\n", __func__, bl_level);
	}
